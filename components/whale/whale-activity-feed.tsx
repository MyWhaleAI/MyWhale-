...
/**
 * Interface for a single transaction item in the activity feed.
 * @interface Transaction
 * @property {number} id - Unique identifier for the transaction.
 * @property {string} wallet_address - The wallet address involved in the transaction.
 * @property {string} signature - The transaction signature.
 * @property {string} action - The type of action (e.g., "Bought", "Staked").
 * @property {string} platform - The platform where the transaction occurred.
 * @property {string} value - The value associated with the transaction (e.g., "$1000 SOL").
 * @property {string} [ai_summary] - An optional AI-generated summary of the transaction.
 * @property {string} timestamp - The timestamp of the transaction in ISO format.
 */
interface Transaction {
  id: number;
...
/**
 * Props for the WhaleActivityFeed component.
 * @interface WhaleActivityFeedProps
 * @property {string} address - The wallet address of the whale whose activity is being displayed.
 * @property {Transaction[]} [transactions=[]] - An array of transaction objects to display in the feed.
 */
interface WhaleActivityFeedProps {
  address: string;
...
/**
 * WhaleActivityFeed component displays a feed of recent on-chain activities for a specific whale.
 * It includes filtering options for different types of transactions (All, Buys, Staking, NFTs).
 *
 * @param {WhaleActivityFeedProps} { address, transactions } - The props object containing whale address and transactions.
...
export function WhaleActivityFeed({ address, transactions = [] }: WhaleActivityFeedProps) {
  const [activeFilter, setActiveFilter] = useState("all"); // State for the active transaction filter

  // Filter activities based on the selected `activeFilter`
  const filteredActivities =
    activeFilter === "all"
      ...
      : transactions.filter((activity) => {
          ...
          return true; // Default to true if filter is not matched (shouldn't happen with defined filters)
        });
    ...
    <Card className="bg-white border-gray-200 rounded-xl shadow-sm overflow-hidden">
      <CardHeader className="flex flex-row items-center justify-between p-4 border-b border-gray-200">
        ...
        <div className="flex items-center gap-2">
          ...
          {/* Filter button (can be used for more advanced filters) */}
      ...
      <CardContent className="p-0">
        <div className="divide-y divide-gray-100">
          {filteredActivities.length > 0 ? (
            // Map through filtered activities and display each one
            filteredActivities.map((activity) => (
              <div key={activity.id} className="p-4 hover:bg-gray-50 transition-colors">
                <div className="flex justify-between items-start mb-2">
                  <div className="flex items-center gap-2">
                    {/* Dynamic emoji icon based on activity type */}
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {/* External link to transaction on Solscan */}
                    <a
            ...
            // Message when no activities are found for the current filters
