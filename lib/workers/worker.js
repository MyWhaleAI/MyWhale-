...
async function analyzeTransaction(signature, trackedWalletPublicKey) {
    ...
    const allTrackedMintsInTx = new Set([...preTokenWalletBalances.map(b => b.mint), ...postTokenWalletBalances.map(b => b.mint)]);
    ...
    for (const mint of allTrackedMintsInTx) {
        ...
        if (preB?.uiTokenAmount?.uiAmountString) preAmt = parseFloat(preB.uiTokenAmount.uiAmountString);
        else if (preB?.uiTokenAmount?.amount) preAmt = parseFloat((BigInt(preB.uiTokenAmount.amount) / BigInt(10**tokenDetails.decimals)).toString());
        if (postB?.uiTokenAmount?.uiAmountString) postAmt = parseFloat(postB.uiTokenAmount.uiAmountString);
        else if (postB?.uiTokenAmount?.amount) postAmt = parseFloat((BigInt(postB.uiTokenAmount.amount) / BigInt(10**tokenDetails.decimals)).toString());

        if (preAmt !== postAmt) {
            balanceChanges.tokens.push({ mint, symbol: tokenDetails.symbol, name: tokenDetails.name, preAmount: preAmt, postAmount: postAmt, change: postAmt - preAmt });
        }
    }
    ...
    if (isLpOrStakingAction) {
        if (involvedPlatforms.has("Marinade Finance")) {
            ...
            if (spentSOL && receivedMSOL) {
                ...
                interpretation = `Interaction with Marinade Finance ${platformContext}. SOL change: ${balanceChanges.sol.toFixed(5)}.`;
            }
            ...
            if (balanceChanges.sol < -0.001) {
                interpretation = `Delegated/Staked SOL or created stake account ${platformContext}. SOL change: ${balanceChanges.sol.toFixed(5)}.`;
                 detailedActions.find(a => a.type === 'DELEGATE_SOL') ? null : detailedActions.push({ type: 'DELEGATE_SOL_GENERIC', amount: Math.abs(balanceChanges.sol) });
                ...
                interpretation = `Withdrew SOL from stake account ${platformContext}. SOL change: ${balanceChanges.sol.toFixed(5)}.`;
                 detailedActions.find(a => a.type === 'WITHDRAW_SOL_STAKE') ? null : detailedActions.push({ type: 'WITHDRAW_SOL_STAKE_GENERIC', amount: balanceChanges.sol });
            } else {
                interpretation = `Interaction with Native SOL Stake program ${platformContext}.`;
            }
        }
        
        else if ( (involvedPlatforms.has("Raydium AMM V4") || involvedPlatforms.has("Orca Swap")) && spentTokens.length >= 1 && receivedTokens.length === 1 && receivedTokens[0].symbol.toLowerCase().includes('lp')) {
        ...
        } else if ( (involvedPlatforms.has("Raydium AMM V4") || involvedPlatforms.has("Orca Swap")) && spentTokens.length === 1 && spentTokens[0].symbol.toLowerCase().includes('lp') && receivedTokens.length >= 1) {
    ...
    else if (interpretation === "No specific action identified for tracked wallet." && (balanceChanges.sol !== 0 || balanceChanges.tokens.length > 0)) {
        let changesSummary = [];
        if (balanceChanges.sol > 0.00001) changesSummary.push(`Received ${balanceChanges.sol.toFixed(5)} SOL`);
        else if (balanceChanges.sol < -0.00001) changesSummary.push(`Sent ${Math.abs(balanceChanges.sol).toFixed(5)} SOL`);
        
        balanceChanges.tokens.forEach(t => {
            if (t.change > 0) changesSummary.push(`Received ${t.change.toFixed(5)} ${t.symbol}`);
            else if (t.change < 0) changesSummary.push(`Sent ${Math.abs(t.change).toFixed(5)} ${t.symbol}`);
        });
        if (changesSummary.length > 0) {
            interpretation = `General balance changes: ${changesSummary.join(', ')}. Involved platforms: ${Array.from(involvedPlatforms).join(', ') || 'Unknown'}.`;
        }
    }
        ...
        connection.onLogs(
            ...
            async (logInfo, context) => {
                ...
                if (err) {
                    ...
                    eventFeed.push({ id: eventFeed.length + 1, time: timestamp, signature, slot: context.slot, status: "LogError", errorDetails: err, rawLogsFromOnLogs: logs });
